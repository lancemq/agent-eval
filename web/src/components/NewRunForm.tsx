...
export function NewRunForm({ draftEvalConfig, clearDraftEvalConfig, onCreated }: Props) {
  ...
  useEffect(() => {
    api.plugins().then((items) => { setPlugins(items); setSelectedPlugins(items.slice(0, 1).map((p) => p.name)) }).catch(console.error)
  ...
  useEffect(() => {
    ...
    setMessage('已载入 Trace 生成的 custom_eval 配置')
    clearDraftEvalConfig()
  }, [draftEvalConfig, clearDraftEvalConfig, runDefaults])

  const parsedConfig = useMemo(() => { try { return JSON.parse(configText) } catch { return null } }, [configText])
  ...
  function applyConfig(nextConfig: any) {
    ...
    if (nextConfig.plugins?.custom_eval && !selectedPlugins.includes('custom_eval')) setSelectedPlugins([...selectedPlugins, 'custom_eval'])
  }

  async function startRun() {
    if (!parsedConfig) return setMessage('配置 JSON 格式错误')
    const mergedPlugins = { ...(parsedConfig.plugins || {}) }
    for (const name of selectedPlugins) mergedPlugins[name] = { enabled: true, ...(mergedPlugins[name] || {}) }
  ...
  return (
    <div className="two-column modal-form">
      <div className="form">
        ...
        <p className="muted">API key 请通过环境变量配置。</p>
      </div>
      <div>
