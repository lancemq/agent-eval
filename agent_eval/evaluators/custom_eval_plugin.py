"""Configuration-driven custom evaluation evaluator."""
...
from agent_eval.evaluators.base import BaseEvaluator, EvalContext, EvalResult, EvaluationType, register_evaluator
from agent_eval.scorers.factory import ScorerFactory


ALLOWED_TASK_SOURCE_TYPES = {"inline", "file", "dataset"}
...
@register_evaluator
class CustomEvalEvaluator(BaseEvaluator):
    name = "custom_eval"
    version = "1.0"
    ...
    def generate_tasks(self, context: EvalContext) -> List[Dict[str, Any]]:
        ...
        for evaluation in self.evaluations:
            ...
            task_items = _load_task_items(evaluation["task_source"], workspace=getattr(context, "workspace", ""))
...
def validate_custom_eval_config(config: Dict[str, Any], workspace: str = None) -> List[Dict[str, str]]:
    ...
    if not isinstance(evaluations, list) or not evaluations:
        return [{"field": "evaluators.custom_eval.evaluations", "message": "evaluations must be a non-empty list"}]

    for eval_index, evaluation in enumerate(evaluations):
        prefix = f"evaluators.custom_eval.evaluations[{eval_index}]"
        ...
        if not isinstance(task_source, dict):
            ...
            if source_type == "dataset":
                ds_name = task_source.get("name")
                if not ds_name:
                    errors.append({"field": f"{prefix}.task_source.name", "message": "dataset task_source requires name"})
                else:
                    try:
                        from agent_eval.datasets import DatasetStore
                        DatasetStore(workspace or os.getcwd()).get(ds_name, task_source.get("version"))
                    except KeyError as exc:
                        errors.append({"field": f"{prefix}.task_source.name", "message": str(exc)})
...
def _load_task_items(task_source: Dict[str, Any], workspace: str = "") -> List[Dict[str, Any]]:
    source_type = task_source.get("type", "inline")
    ...
    if source_type == "dataset":
        ...
        version = task_source.get("version")
        store = DatasetStore(workspace or os.getcwd())
