...
from agent_eval.config import EvaluationConfig, EvaluatorConfig
from agent_eval.runner import agent_spec_from_config, create_agent, import_evaluator_modules, resolve_evaluator_names, run_evaluation_from_config
...
def test_resolve_plugin_names_prefers_override():
    config = EvaluationConfig(evaluators={"a": EvaluatorConfig(enabled=True), "b": EvaluatorConfig(enabled=False)})
    assert resolve_evaluator_names(config, ["b"]) == ["b"]


def test_resolve_plugin_names_uses_enabled_plugins():
    config = EvaluationConfig(evaluators={"a": EvaluatorConfig(enabled=True), "b": EvaluatorConfig(enabled=False)})
    assert resolve_evaluator_names(config) == ["a"]
...
def test_import_plugin_modules(tmp_path):
    ...
    try:
        import_evaluator_modules(["sample_plugin_module"])
...
def test_run_evaluation_from_config(mock_create_agent, mock_orchestrator_cls, mock_generator_cls, tmp_path):
    config = EvaluationConfig(evaluators={"mock": EvaluatorConfig(enabled=True, config={"x": 1})})
    config.agent.config["model"] = "gpt-test"
    report = MagicMock()
